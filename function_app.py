...
def read_flexible_csv(file_handle):
    ...
    for i, row in enumerate(reader):
        if i == 1:  # First line contains some metadata
            ...
            #if len(row) >= (len(columns) - 4 + 2):  # Since we already have first 4 columns from metadata
            new_row = [
...
def should_exclude_file(filename):
    ...
    exclude_patterns = [r'Transformador', r'MONTACARGAS']
